...
async function fetchVideoLibrary(authInfo, onBatch) {
  ...
  // IMPORTANT: Use window.fetch (SES-enhanced) NOT __originalFetch.
  // SES's hardened fetch adds auth headers that the Sora backend requires.
  // Using the pre-SES fetch results in 401 Unauthorized.
  ...
  // Quick probe helper — single attempt, uses SES-enhanced fetch for auth
