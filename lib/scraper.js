...
async function extractAuthTokenAsync() {
  // Return cached token if it's a valid JWT
  if (window.__soraAuthToken && window.__soraAuthToken.token &&
    window.__soraAuthToken.token.startsWith('eyJ')) {
    return window.__soraAuthToken;
  }

  // Call /api/auth/session FIRST — this is the authoritative source.
  // The sync strategies (localStorage, cookies) often return wrong values
  // like UUIDs that aren't the real access token.
  try {
    var resp = await window.fetch('/api/auth/session', { credentials: 'include' });
    if (resp.ok) {
      var data = await resp.json();
      var token = data?.accessToken || null;
      if (token && typeof token === 'string' && token.startsWith('eyJ')) {
        console.log('[SoraArchiver] Got JWT from /api/auth/session (' + token.substring(0, 30) + '...)');
        window.__soraAuthToken = { token: token, type: 'bearer' };
        return window.__soraAuthToken;
      }
    }
  ...
  console.warn('[SoraArchiver] Could not get JWT access token. Make sure you are logged in.');
  return null;
}
...
async function fetchVideoLibrary(authInfo, onBatch) {
  // Sora API page size — the real app uses 16, but we can request more.
  // Use 20 to stay reasonable while reducing round trips.
  const LIMIT = 20;
  const MAX_RETRIES = 3;
  ...
  // Build request headers matching what the Sora app sends
  function buildHeaders() {
    const headers = {};
    if (authInfo && authInfo.token && authInfo.type === 'bearer') {
      headers['Authorization'] = 'Bearer ' + authInfo.token;
    }
    // oai-device-id is required — extract from the oai-did cookie
    try {
      var didMatch = document.cookie.match(/oai-did=([^;]+)/);
      if (didMatch) {
        headers['oai-device-id'] = didMatch[1];
      }
    } catch (_) { }
  ...
  // All endpoints to scan — we fetch ALL of them, not just the first working one
  const endpoints = [
    origin + '/backend/project_y/profile_feed/me',
    origin + '/backend/project_y/profile/drafts/v2',
  ];

  console.log('[SoraArchiver] Scanning ' + endpoints.length + ' endpoint(s)...');

  // Scan each endpoint fully (all pages), then move to the next
  for (const endpoint of endpoints) {
    cursor = null;
    console.log('[SoraArchiver] Scanning endpoint: ' + endpoint);

    // Main pagination loop for current endpoint
    do {
      const url = cursor
        ? `${endpoint}?limit=${LIMIT}&cut=nf2&cursor=${encodeURIComponent(cursor)}`
        : `${endpoint}?limit=${LIMIT}&cut=nf2`;
      ...
      // Advance cursor — Sora API uses data.cursor; null means no more pages
      cursor = data.cursor || data.next_cursor || data.after || null;

      console.log('[SoraArchiver] Page fetched: ' + normalized.length + ' videos (total: ' + totalCount + '), cursor: ' + (cursor ? 'yes' : 'no'));
...
function normalizePostData(item) {
  ...
  return {
    ...
    video_url: (attachment.download_urls && attachment.download_urls.no_watermark) || attachment.downloadable_url || (attachment.download_urls && attachment.download_urls.watermark) || attachment.url || null,
    has_audio: null,
...
function normalizeVideoData(raw) {
  ...
  const video_url =
    (attachment?.download_urls && attachment.download_urls.no_watermark) ||
    attachment?.downloadable_url ||
    (attachment?.download_urls && attachment.download_urls.watermark) ||
    attachment?.url ||
    v.video_url ||
    v.mp4_url ||
    v.download_url ||
