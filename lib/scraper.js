...
function interceptFetch(onVideoData) {
  ...
  try {
    var perfObserver = new PerformanceObserver(function(list) {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        // Re-fetch the URL to read the response (the browser may serve from cache)
        (function(fetchUrl) {
  ...
  // -------------------------------------------------------------------
  // Strategy 2: Fetch monkey-patch (best-effort, no SES retry)
  // -------------------------------------------------------------------
  // NOTE: OpenAI uses SES (Secure EcmaScript) lockdown which overwrites
  // window.fetch after our script runs. Re-patching after SES causes
  // infinite recursion (SES's wrapper references window.fetch internally).
  // We install the patch once — it works until SES runs. After that,
  // PerformanceObserver and DOM scraping handle discovery.
  try {
    ...
    console.log('[SoraArchiver] Fetch patch installed (will be replaced by SES — that is OK).');
  } catch (e) {
