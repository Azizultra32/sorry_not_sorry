...
async function fetchVideoLibrary(authInfo, onBatch) {
  ...
  for (const endpoint of uniqueEndpoints) {
    try {
      ...
      // Accept if response looks like a video list (including real Sora feed format)
      const isPostFeed = Array.isArray(data.items) && data.items.length > 0 &&
                         data.items[0].post && data.items[0].post.attachments;
      if (data && (isPostFeed ||
                   Array.isArray(data.data) || Array.isArray(data.videos) ||
  ...
  do {
    // Sora API uses 'cursor'; legacy APIs may use 'after'
    const url = cursor
      ? `${workingEndpoint}?limit=${LIMIT}&cursor=${encodeURIComponent(cursor)}`
  ...
  try {
    var perfObserver = new PerformanceObserver(function(list) {
      ...
      for (var i = 0; i < entries.length; i++) {
        ...
        // API response interception — only re-fetch same-origin URLs
        // (cross-origin re-fetches fail with CORS when using credentials)
        ...
        // Re-fetch the same-origin URL to read the response
        (function(fetchUrl) {
          var doFetch = window.__originalFetch || window.fetch.bind(window);
          doFetch(fetchUrl, { credentials: 'same-origin' })
            .then(function(resp) {
              if (!resp.ok) return;
